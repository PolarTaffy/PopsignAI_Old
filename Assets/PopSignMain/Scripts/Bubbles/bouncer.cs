...
public class bouncer : MonoBehaviour
{
    ...
    {
        if (transform.position.y >= 530f / 640f *UnityEngine.Camera.main.orthographicSize)
        {
           UnityEngine.Camera.main.GetComponent<mainscript>().controlArray = union(b,UnityEngine.Camera.main.GetComponent<mainscript>().controlArray);
            b.Clear();
            return true;    /// don't destroy
        }
        if (UnityEngine.Camera.main.GetComponent<mainscript>().controlArray.Contains(gameObject)) { b.Clear(); return true; } /// don't destroy
        b.Add(gameObject);
