...
//				pointers2[i].GetComponent<SpriteRenderer>().color = col;
    ...
    {
        GeneratePositionsPoints();
        if(Input.GetMouseButton(0) && (UnityEngine.Camera.main.ScreenToWorldPoint(Input.mousePosition).y > -4.1f || arrowDown))
        {

            if(!arrowDown)
                dir = UnityEngine.Camera.main.ScreenToWorldPoint(Input.mousePosition) - Vector3.back * 10;
