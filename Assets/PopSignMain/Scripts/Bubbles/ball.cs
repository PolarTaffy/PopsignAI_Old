...
public class ball : MonoBehaviour
{
    ...
    private RandomVideoSelector randomVideo;
    ...
    void Start()
    {
        ...
        bottomBorder =UnityEngine.Camera.main.GetComponent<mainscript>().bottomBorder;
        topBorder =UnityEngine.Camera.main.GetComponent<mainscript>().topBorder;
        leftBorder =UnityEngine.Camera.main.GetComponent<mainscript>().leftBorder;
        rightBorder =UnityEngine.Camera.main.GetComponent<mainscript>().rightBorder;
        isPaused =UnityEngine.Camera.main.GetComponent<mainscript>().isPaused;
        ...
        if(!GamePlay.Instance.isPopSignAI) randomVideo = GameObject.Find("Canvas/HideBalls/SignVideos/Video").GetComponent<RandomVideoSelector>();


        if (GamePlay.Instance.isPopSignAI) {
            engine = GameObject.Find("SimpleSLREngine(NoCanvas)").GetComponent<SimpleExecutionEngine>();
            engine.recognizer.AddCallback("Sign", Shoot);
        }
    }
    ...
    void Update()
    {
        ...
        if (launched && !gameObject.GetComponent<ball>().setTarget &&
            ...
            newBall && !UnityEngine.Camera.main.GetComponent<mainscript>().gameOver &&
        ...
        if (!launched && !gameObject.GetComponent<ball>().setTarget &&
            ...
            (GamePlay.Instance.GameStatus == GameState.Playing ||
                GamePlay.Instance.GameStatus == GameState.WaitForStar))
        {
            // UnityEngine.Debug.Log(sharedVideoManager);
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (!launched && !ball.GetComponent<ball>().setTarget && mainscript.Instance.newBall2 == null &&
                !UnityEngine.Camera.main.GetComponent<mainscript>().gameOver &&
                (GamePlay.Instance.GameStatus == GameState.Playing || GamePlay.Instance.GameStatus == GameState.WaitForStar))
            {
                ...
                if (!mainscript.StopControl)
                {
                    ...
                    if (signShoot != null && signShoot.activeSelf) {
                        if (signShoot.GetComponent<HoldToSign>().fingerLifted && UnityEngine.Camera.main.ScreenToWorldPoint(Input.mousePosition).y <= -4.1f) {
                            StopRecordingAndShoot();
                            signShoot.GetComponent<HoldToSign>().fingerLifted = false;
                        }
                    } 
                    else {
                        StopRecordingAndShoot();
        ...
        if (transform.position != target && setTarget && !stoppedBall && !isPaused &&UnityEngine.Camera.main.GetComponent<mainscript>().dropDownTime < Time.time)
        {
            float totalVelocity = Vector3.Magnitude(GetComponent<Rigidbody2D>().linearVelocity);
            if (totalVelocity > 20)
            {
                float tooHard = totalVelocity / (20);
                GetComponent<Rigidbody2D>().linearVelocity /= tooHard;
            ...
            else if (totalVelocity < 15)
            {
                ...
                if (tooSlowRate != 0)
                    GetComponent<Rigidbody2D>().linearVelocity /= tooSlowRate;
            }

            if (GetComponent<Rigidbody2D>().linearVelocity.y < 1.5f && GetComponent<Rigidbody2D>().linearVelocity.y > 0)
                GetComponent<Rigidbody2D>().linearVelocity = new Vector2(GetComponent<Rigidbody2D>().linearVelocity.x, 1.7f);
        }
    ...
    public void StopRecordingAndShoot()
    {
        ...
        if(GamePlay.Instance.isPopSignAI)
        {
            // result = TfLiteManager.Instance.StopRecording();
            UnityEngine.Debug.Log("Triggered");
            engine.buffer.TriggerCallbacks();
        }
        else
        {
            Shoot(randomVideo.reviewWord);
        }
    }

    public void Shoot(string result) {
       ...
       UnityEngine.Camera.main.GetComponent<TutorialManager>().BallHit();

        if(!GamePlay.Instance.isPopSignAI)
        {
            randomVideo.RandomizeVideo();
        }    
    }

    void FixedUpdate()
    {
        if (UnityEngine.Camera.main.GetComponent<mainscript>().gameOver) return;
        ...
        if (stoppedBall)
        {
            ...
            if (newBall)
            {
                ...
                gameObject.layer = 9;
               UnityEngine.Camera.main.GetComponent<mainscript>().flyingBall = gameObject;
                this.enabled = false;
            }
        }
    }
    ...
    public void checkNearestColor()
    {
        ...
        if (ballsToClear.Count < 3)
        {
           UnityEngine.Camera.main.GetComponent<mainscript>().bounceCounter++;
            mainscript.Instance.ComboCount = 0;
        }

       UnityEngine.Camera.main.GetComponent<mainscript>().droppingDown = false;
    ...
    public void StartFall()
    {
        ...
        gameObject.GetComponent<Rigidbody2D>().linearVelocity = gameObject.GetComponent<Rigidbody2D>().linearVelocity + new Vector2(Random.Range(-2, 2), 0);
    ...
    public bool checkNearestBall(ArrayList b)
    {
        ...
        if (UnityEngine.Camera.main.GetComponent<mainscript>().controlArray.Contains(gameObject))
        {
            b.Clear();
            return true; // don't destroy
        }
            ...
            if (obj != gameObject && obj != null)
            {
                if (obj.gameObject.layer == 9) // ball layer
                {
                    ...
                    if (distanceToBall <= 1.0f && distanceToBall > 0)
                    {
                        ...
                        {
                           UnityEngine.Camera.main.GetComponent<mainscript>().arraycounter++;
    ...
    IEnumerator DestroyCor(ArrayList b, float speed = 0.1f)
    {
       ...
       UnityEngine.Camera.main.GetComponent<mainscript>().bounceCounter = 0;
        //int scoreCounter = 0;
        int soundPool = 0;
        foreach (GameObject obj in l)
        {
            ...
            //UnityEngine.Camera.main.GetComponent<mainscript>().explode( obj.gameObject );
