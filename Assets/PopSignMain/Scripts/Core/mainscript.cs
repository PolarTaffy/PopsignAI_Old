...
public class mainscript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Debug.Log("First else if wiht no new ball");
    ...
    public IEnumerator clearDisconnectedBalls()
    {
        ...
        int willDestroy = 0;
        // destringAloneBall = true;
        UnityEngine.Camera.main.GetComponent<mainscript>().arraycounter = 0;
        GameObject[] fixedBalls = GameObject.FindObjectsOfType(typeof(GameObject)) as GameObject[]; // detect alone balls
        UnityEngine.Camera.main.GetComponent<mainscript>().controlArray.Clear();
        foreach (GameObject obj in fixedBalls)
        {
            if (obj != null)
            {
